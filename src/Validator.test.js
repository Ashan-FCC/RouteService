...
describe('Validator', () => {
  ...
  describe('validate input for ', () => {
    const testCases = [
      {start: 'A', end: 'B', maxStops: 1, validates: true},
      {start: '1', end: 'B', maxStops: 1, validates: false},
      {start: 'A', end: 'B', maxStops: 'A', validates: false},
      {start: {}, end: 'B', maxStops: 1, validates: false}
    ]
    testCases.forEach(({start, end, maxStops, validates}) => {
      it(`should validate input: ${start} - ${end} - ${maxStops}to be ${validates}`, () => {
        if (validates) {
          assert.doesNotThrow(() => validateInput(start, end, maxStops))
        } else {
          assert.throws(() => validateInput(start, end, maxStops), ValidationError)
        }
      })
    })
  })
